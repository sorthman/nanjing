<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-select
        v-model="listQuery.glyy"
        clearable
        style="width: 150px"
        class="filter-item"
        placeholder="隔离原因"
      >
        <el-option v-for="(key, value) in glyyMap" :key="key" :label="key" :value="value" />
      </el-select>
      <el-select
        v-model="listQuery.zdyq"
        clearable
        style="width: 150px"
        class="filter-item"
        placeholder="重点疫区"
      ...
      ></el-date-picker>
      <el-select
        v-model="listQuery.stopinfo"
        clearable
        style="width: 150px"
        class="filter-item"
        placeholder="中止隔离原因"
      >
        <el-option v-for="(key, value) in stopinfoMap" :key="key" :label="key" :value="value" />
      </el-select>
      <el-date-picker
    ...
    >
      ...
      <el-table-column align="center" label="行政区" prop="area" />
      <el-table-column align="center" label="街道" prop="street" />
      <el-table-column align="center" label="社区" prop="community" />

      <el-table-column align="center" label="常驻地址" min-width="300px" prop="hjaddress" />
      <el-table-column align="center" label="常驻地址" min-width="300px" prop="liveaddress" />

      <el-table-column align="center" label="隔离原因" prop="glinfo" />
      <el-table-column align="center" label="重点疫区说明" prop="zdyqinfo" />
      <el-table-column align="center" label="隔离原因其他说明" prop="glqtinfo" />
      <el-table-column align="center" min-width="120px" label="开始居家隔离时间" prop="arrivedate">
        <template slot-scope="scope">{{scope.row.starttime?scope.row.starttime.substring(0,10):""}}</template>
      </el-table-column>
      <el-table-column align="center" min-width="120px" label="中止居家隔离时间" prop="arrivedate">
        <template slot-scope="scope">{{scope.row.stoptime?scope.row.stoptime.substring(0,10):""}}</template>
      </el-table-column>
      <el-table-column align="center" label="中止隔离原因" prop="stopinfo" />
      <el-table-column align="center" min-width="120px" label="解除居家隔离时间" prop="arrivedate">
        <template slot-scope="scope">{{scope.row.endtime?scope.row.endtime.substring(0,10):""}}</template>
      </el-table-column>
      <el-table-column align="center" label="社工" prop="sgname" />
      <el-table-column align="center" label="网格人员" prop="wgname" />
...
<script>
import { listUser } from "@/api/njuser";
