<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button v-permission="['GET /adminapi/vip/list']" class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">查找</el-button>
      <el-button v-permission="['POST /adminapi/vip/create']" class="filter-item" type="primary" icon="el-icon-edit" @click="handleCreate">添加</el-button>
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column fixed="right" align="center" label="操作" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-permission="['POST /adminapi/vip/update']" type="primary" size="mini" @click="handleUpdate(scope.row)">编辑</el-button>
          <el-button v-permission="['POST /adminapi/vip/delete']" type="danger" size="mini" @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="图书类型" prop="typename">
          <el-select v-model="dataForm.typename" clearable style="width: 200px" class="filter-item" placeholder="请选择图书类型">
            <el-option v-for="(key, value) in typeMap" :key="key" :label="key" :value="value"/>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="是否新品" prop="isnew">
          <el-select v-model="dataForm.isnew" clearable style="width: 200px" class="filter-item" placeholder="请选择图书类型">
            <el-option v-for="(key, value) in statusMap" :key="key" :label="key" :value="value"/>
          </el-select>
        </el-form-item>
        <el-form-item label="是否推荐" prop="iscommend">
          <el-select v-model="dataForm.iscommend" clearable style="width: 200px" class="filter-item" placeholder="请选择图书类型">
            <el-option v-for="(key, value) in statusMap" :key="key" :label="key" :value="value"/>
          </el-select>
        </el-form-item>
        <el-form-item label="是否热销" prop="ishot">
          <el-select v-model="dataForm.ishot" clearable style="width: 200px" class="filter-item" placeholder="请选择图书类型">
            <el-option v-for="(key, value) in statusMap" :key="key" :label="key" :value="value"/>
          </el-select>
        </el-form-item>
